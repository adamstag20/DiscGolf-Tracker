...
class MetadataValidator {
    ...
    validateEntityDefinition(metadata, name) {
        ...
        if (meta.virtual || meta.expression) {
            for (const prop of Object.values(meta.properties)) {
                if (![enums_1.ReferenceType.SCALAR, enums_1.ReferenceType.EMBEDDED].includes(prop.reference)) {
                    throw new errors_1.MetadataError(`Only scalar and embedded properties are allowed inside virtual entity. Found '${prop.reference}' in ${meta.className}.${prop.name}`);
                }
                if (prop.primary) {
    ...
    validateDiscovered(discovered, warnWhenNoEntities) {
        ...
        const duplicateTableNames = utils_1.Utils.findDuplicates(tableNames.map(meta => meta.tableName || meta.collection));
        ...
        discovered.forEach(meta => Object.values(meta.properties).forEach(prop => {
            if (prop.reference !== enums_1.ReferenceType.SCALAR && !unwrap(prop.type).split(/ ?\| ?/).every(type => discovered.find(m => m.className === type))) {
