...
class UnitOfWork {
    ...
    registerManaged(entity, data, options) {
        ...
        wrapped.__em ?? (wrapped.__em = this.em);
    ...
    getById(entityName, id, schema) {
        ...
        let hash = Array.isArray(id) ? Utils_1.Utils.getPrimaryKeyHash(Utils_1.Utils.flatten(id)) : '' + id;
    ...
    remove(entity, visited, options = {}) {
        ...
        for (const prop of (0, entity_1.helper)(entity).__meta.bidirectionalRelations) {
            ...
            if (prop.reference === enums_1.ReferenceType.ONE_TO_MANY && prop2.nullable && Utils_1.Utils.isCollection(relation)) {
    ...
    unsetIdentity(entity) {
        ...
        for (const prop of wrapped.__meta.bidirectionalRelations) {
            ...
            const rel = entity_1.Reference.unwrapReference(entity[prop.name]);
            if (Utils_1.Utils.isCollection(rel) && rel.isInitialized()) {
                rel.getItems(false).forEach(item => rel.removeWithoutPropagation(item));
                continue;
            }
            if (Utils_1.Utils.isCollection(rel?.[inverse]) && rel[inverse].isInitialized()) {
            ...
            if (rel?.[inverse] && entity === entity_1.Reference.unwrapReference(rel[inverse])) {
                delete (0, entity_1.helper)(rel).__data[inverse];
            }
        }
    ...
    computeChangeSets() {
        ...
        for (const entity of this.removeStack) {
            ...
            const deletePkHash = [(0, entity_1.helper)(entity).getSerializedPrimaryKey(), ...this.expandUniqueProps(entity)];
    ...
    expandUniqueProps(entity) {
        ...
        return wrapped.__meta.uniqueProps.map(prop => {
            if (entity[prop.name]) {
                return prop.reference === enums_1.ReferenceType.SCALAR || prop.mapToPk ? entity[prop.name] : (0, entity_1.helper)(entity[prop.name]).getSerializedPrimaryKey();
            }
    ...
    processReference(parent, prop, reference, visited, idx) {
        ...
        if (Utils_1.Utils.isCollection(reference)) {
            reference.getItems(false)
                .filter(item => !item.__helper.__originalEntityData)
                .forEach(item => {
                var _a;
                // propagate schema from parent
                (_a = item.__helper).__schema ?? (_a.__schema = (0, entity_1.helper)(parent).__schema);
            });
            if (prop.reference === enums_1.ReferenceType.MANY_TO_MANY && reference.isDirty()) {
    ...
    fixMissingReference(entity, prop) {
        ...
        if ([enums_1.ReferenceType.MANY_TO_ONE, enums_1.ReferenceType.ONE_TO_ONE].includes(prop.reference) && reference && !prop.mapToPk) {
            ...
            else if (!(0, entity_1.helper)(reference).__initialized && !(0, entity_1.helper)(reference).__em) {
                const pk = (0, entity_1.helper)(reference).getPrimaryKey();
